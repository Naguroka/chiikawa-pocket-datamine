...
public final class ChannelMixingMatrix {
    ...
    public static androidx.media3.common.audio.ChannelMixingMatrix create(int i, int i2) {
        return new androidx.media3.common.audio.ChannelMixingMatrix(i, i2, createMixingCoefficients(i, i2));
    }

    public ChannelMixingMatrix(int i, int i2, float[] fArr) {
        androidx.media3.common.util.Assertions.checkArgument(i > 0, "Input channel count must be positive.");
        androidx.media3.common.util.Assertions.checkArgument(i2 > 0, "Output channel count must be positive.");
        androidx.media3.common.util.Assertions.checkArgument(fArr.length == i * i2, "Coefficient array length is invalid.");
    ...
    public androidx.media3.common.audio.ChannelMixingMatrix scaleBy(float f) {
    ...
    private static float[] createMixingCoefficients(int i, int i2) {
        ...
        throw new java.lang.UnsupportedOperationException("Default channel mixing coefficients for " + i + "->" + i2 + " are not yet implemented.");
    }
    ...
    private static float[] checkCoefficientsValid(float[] fArr) {
        for (int i = 0; i < fArr.length; i++) {
            if (fArr[i] < 0.0f) {
                throw new java.lang.IllegalArgumentException("Coefficient at index " + i + " is negative.");
            }
        }
        return fArr;
    }
