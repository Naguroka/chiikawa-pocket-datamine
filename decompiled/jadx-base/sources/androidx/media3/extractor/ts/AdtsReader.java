...
public final class AdtsReader implements androidx.media3.extractor.ts.ElementaryStreamReader {
    ...
    public AdtsReader(boolean z, java.lang.String str, int i) {
        ...
        this.sampleDurationUs = androidx.media3.common.C.TIME_UNSET;
    ...
    public void createTracks(androidx.media3.extractor.ExtractorOutput extractorOutput, androidx.media3.extractor.ts.TsPayloadReader.TrackIdGenerator trackIdGenerator) {
        ...
        androidx.media3.extractor.TrackOutput trackOutputTrack = extractorOutput.track(trackIdGenerator.getTrackId(), 1);
        this.output = trackOutputTrack;
        this.currentOutput = trackOutputTrack;
        if (this.exposeId3) {
            trackIdGenerator.generateNewId();
            androidx.media3.extractor.TrackOutput trackOutputTrack2 = extractorOutput.track(trackIdGenerator.getTrackId(), 5);
            this.id3Output = trackOutputTrack2;
            trackOutputTrack2.format(new androidx.media3.common.Format.Builder().setId(trackIdGenerator.getFormatId()).setSampleMimeType(androidx.media3.common.MimeTypes.APPLICATION_ID3).build());
            return;
        }
    ...
    private void parseAdtsHeader() throws androidx.media3.common.ParserException {
        ...
        if (!this.hasOutputFormat) {
            ...
            if (bits != 2) {
                androidx.media3.common.util.Log.w(TAG, "Detected audio object type: " + bits + ", but assuming AAC LC.");
                bits = 2;
            }
            this.adtsScratch.skipBits(5);
            byte[] bArrBuildAudioSpecificConfig = androidx.media3.extractor.AacUtil.buildAudioSpecificConfig(bits, this.firstFrameSampleRateIndex, this.adtsScratch.readBits(3));
            androidx.media3.extractor.AacUtil.Config audioSpecificConfig = androidx.media3.extractor.AacUtil.parseAudioSpecificConfig(bArrBuildAudioSpecificConfig);
            androidx.media3.common.Format formatBuild = new androidx.media3.common.Format.Builder().setId(this.formatId).setSampleMimeType(androidx.media3.common.MimeTypes.AUDIO_AAC).setCodecs(audioSpecificConfig.codecs).setChannelCount(audioSpecificConfig.channelCount).setSampleRate(audioSpecificConfig.sampleRateHz).setInitializationData(java.util.Collections.singletonList(bArrBuildAudioSpecificConfig)).setLanguage(this.language).setRoleFlags(this.roleFlags).build();
    ...
    @org.checkerframework.checker.nullness.qual.EnsuresNonNull({org.jacoco.core.runtime.AgentOptions.OUTPUT, "currentOutput", "id3Output"})
