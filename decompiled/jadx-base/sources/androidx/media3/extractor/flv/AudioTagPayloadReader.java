...
final class AudioTagPayloadReader extends androidx.media3.extractor.flv.TagPayloadReader {
    ...
    protected boolean parseHeader(androidx.media3.common.util.ParsableByteArray parsableByteArray) throws androidx.media3.extractor.flv.TagPayloadReader.UnsupportedFormatException {
        if (!this.hasParsedAudioDataHeader) {
            ...
            this.audioFormat = i;
            if (i == 2) {
                this.output.format(new androidx.media3.common.Format.Builder().setSampleMimeType(androidx.media3.common.MimeTypes.AUDIO_MPEG).setChannelCount(1).setSampleRate(AUDIO_SAMPLING_RATE_TABLE[(unsignedByte >> 2) & 3]).build());
                this.hasOutputFormat = true;
            } else if (i == 7 || i == 8) {
                this.output.format(new androidx.media3.common.Format.Builder().setSampleMimeType(i == 7 ? androidx.media3.common.MimeTypes.AUDIO_ALAW : androidx.media3.common.MimeTypes.AUDIO_MLAW).setChannelCount(1).setSampleRate(8000).build());
                this.hasOutputFormat = true;
            } else if (i != 10) {
    ...
    protected boolean parsePayload(androidx.media3.common.util.ParsableByteArray parsableByteArray, long j) throws androidx.media3.common.ParserException {
        ...
        if (unsignedByte == 0 && !this.hasOutputFormat) {
            ...
            this.output.format(new androidx.media3.common.Format.Builder().setSampleMimeType(androidx.media3.common.MimeTypes.AUDIO_AAC).setCodecs(audioSpecificConfig.codecs).setChannelCount(audioSpecificConfig.channelCount).setSampleRate(audioSpecificConfig.sampleRateHz).setInitializationData(java.util.Collections.singletonList(bArr)).build());
            this.hasOutputFormat = true;
            return false;
        }
